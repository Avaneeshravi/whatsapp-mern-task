...
function App() {
  ...
  const handleProfilePicChange = (e) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        const res = await axios.put(`http://localhost:5000/api/users/${user}/profile`, payload);
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      try {
        const grpRes = await axios.get(`http://localhost:5000/api/groups/${user}`);
        allUsers = [...allUsers, ...grpRes.data];
      } catch (e) { }
      setUsers(allUsers);

      try {
        const statRes = await axios.get('http://localhost:5000/api/status');
  ...
  useEffect(() => {
    if (selectedUser && user) {
      ...
      axios.get(`http://localhost:5000/api/messages/${user}/${selectedUser}`)
        .then(res => {
  ...
  useEffect(() => {
    if (viewingStatuses && viewingStatuses.username !== user) {
      ...
      if (currentStatus && !currentStatus.viewers?.includes(user)) {
        axios.put('http://localhost:5000/api/status/view', {
  ...
  const handleExitGroup = async () => {
    if (window.confirm(`Exit this group?`)) {
      try {
        await axios.put(`http://localhost:5000/api/groups/${selectedUser}/exit`, { username: user });
  ...
  const handleAddMember = async (newUsername) => {
    try {
      await axios.put(`http://localhost:5000/api/groups/${selectedUser}/add`, { newMembers: [newUsername] });
  ...
  const handleStatusUpload = async (type, content, bgColor = null, caption = '') => {
    try {
      await axios.post('http://localhost:5000/api/status', { username: user, type, content, bgColor, caption });
      fetchUsers();
      let statRes = await axios.get('http://localhost:5000/api/status');
      setStatuses(statRes.data);
